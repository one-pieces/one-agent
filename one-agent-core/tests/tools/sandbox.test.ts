...
import { ToolRegistry, bashTool } from "../../src/tools/index.ts";

describe("工具沙箱（worker_threads 隔离）", () => {
  const registry = new ToolRegistry();
  registry.add(bashTool);
  ...
  it("沙箱内正常执行（bash 标记 sandbox）", async () => {
    const res = await registry.execute({}, { id: "c1", name: "bash", input: { command: "echo hi" } });
  ...
  it("LLM 未传 cwd → 默认在 ctx.cwd（会话工作区）执行", async () => {
    ...
    dirs.push(ws);
    const res = await registry.execute({ cwd: ws }, { id: "c1", name: "bash", input: { command: "pwd" } });
    expect(res.ok).toBe(true);
  ...
  it("LLM 显式传 cwd → 覆盖 ctx.cwd", async () => {
    ...
    dirs.push(ws, other);
    const res = await registry.execute({ cwd: ws }, { id: "c1", name: "bash", input: { command: "pwd", cwd: other } });
    expect(res.ok).toBe(true);
    expect(realpathSync((res.output as { stdout: string }).stdout.trim())).toBe(realpathSync(other));
  });

  it("命令失败 → ok:false（进程内返回，不崩溃）", async () => {
    const res = await registry.execute({}, { id: "c1", name: "bash", input: { command: "exit 3" } });
    expect(res.ok).toBe(false);
    expect(res.error).toContain("执行失败");
  });

  it("长命令超时 → 优雅失败（worker/exec 超时，主进程不受影响）", async () => {
    const res = await registry.execute({}, { id: "c1", name: "bash", input: { command: "sleep 10", timeoutMs: 800 } });
    expect(res.ok).toBe(false);
